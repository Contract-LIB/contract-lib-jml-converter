...
// TODO: WIP! Implementation at the moment identical to OuterViewTranslator!
/**
 * This class can be used to create the "inner view" of a data structure. This view consists of a
 * class containing all the declared abstractions as ghost fields and the methods with the declared
 * contracts as in Contract-LIB. In addition, it contains multiple placeholders (for example for the
 * method bodies and the coupling invariants) which are to be filled in by the user.
 *
 * @author Wolfram Pfeifer
 */
public class InnerViewTranslator {
    private final List<Command> commands;
    private final Map<String, CompilationUnit> classes = new HashMap<>();

    private InnerViewTranslator(List<Command> commands) {
        this.commands = commands;
    }

    static String translate(
        ContractLibANTLRParser<Term, Type, Abstraction, Datatype, FunDec, Command> converter) {

        InnerViewTranslator translator = new InnerViewTranslator(converter.getCommands());
        // translate the declared abstractions
        translator.convertAbstractionCommands();

        // translate the decined contracts (depends on the abstractions!)
